...
class Game:
    ...
    def init_players(self):
        for i in range(self._play_number):        
            ...
            self._players[i] = Player.Player(i, name, symbol)
    
    def convert_to_board_index(self, x, y):
        return x-1, y-1
    
    def place_piece(self, player: Player.Player, board: Board.Board) -> bool:
        x = input("The x place you want to do: ")
        y = input("The y place you want to do: ")
        x_in, y_in = self.convert_to_board_index(x, y)
        if(board.check_available(x_in, y_in)): 
            board.set_piece(x_in, y_in, player)
            return True
        return False
